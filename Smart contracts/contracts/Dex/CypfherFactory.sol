...
contract CyfherFactory is ICyfherFactory {
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        ...
        require(getPair[token0][token1] == address(0), "CyfherSwap: PAIR_EXISTS"); // single check is sufficient

        pair = address(new CyfherPair{ salt: keccak256(abi.encodePacked(token0, token1)) }());
