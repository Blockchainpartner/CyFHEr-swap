...
import privateBgpLogo from '../assets/private bgp.png'

function Liquidity() {
    const logos = [privateEuroLogo
        , privateUsdLogo
        , privateBgpLogo]
    const tokens = [
        { id: 0, name: 'pEUR' },
    ...
    const pairs = [
        { id: 0, name1: 'pEUR', name2: 'pUSD', logo1: privateEuroLogo, logo2: privateUsdLogo },
        { id: 1, name1: 'pEUR', name2: 'pGBP', logo1: privateEuroLogo, logo2: privateBgpLogo },
        { id: 2, name1: 'pUSD', name2: 'pGBP', logo1: privateUsdLogo, logo2: privateBgpLogo },
    ]
    ...
    const handleAddLiquidity = async () => {
        console.log(`Adding liquidity: ${amountA} ${tokenA.name} + ${amountB} ${tokenB.name}`);
    };
    ...
    return (<>
        ...
        before:absolute before:inset-0 before:rounded-xl before:blur-lg before:bg-purple-600 before:opacity-50 before:-z-10">
            <h1 className="text-2xl font-semibold text-white text-center">Add Liquidity</h1>
            ...
            <div className="flex items-center space-x-4">
                <Listbox value={tokenA} onChange={OnChangeTokenA}>
                    ...
                    >
                        <img src={logos[tokenA.id]} alt={tokenA.name} className="w-6 h-6" />
                        <span>{tokenA.name}</span>
                        <ChevronDownIcon className="absolute top-2.5 right-2.5 size-4 fill-white/60" aria-hidden="true" />
                    </ListboxButton>
                    <ListboxOptions
                        anchor="bottom"
                        transition
            ...
            <div className="flex items-center space-x-4">
                <Listbox value={tokenB} onChange={OnChangeTokenB}>
                    ...
                    >
                        <img src={logos[tokenB.id]} alt={tokenB.name} className="w-6 h-6" />
                    ...
                    >
                        {tokens.filter((token) => token.name !== tokenA.name).map((token) => (
                            <ListboxOption
                                key={token.id}
                                value={token}
                                className="group flex items-center gap-2 cursor-default rounded-lg py-1.5 px-3 select-none data-[focus]:bg-white/10"
        ...
        <div className="md:w-3/5 w-full bg-black text-white  px-5 py-6 rounded-xl shadow-lg  relative border border-purple-800 
        before:absolute before:inset-0 before:rounded-xl before:blur-lg before:bg-purple-600 before:opacity-50 before:-z-10">
            <h1 className="text-2xl font-semibold text-white text-center">Remove liquidity </h1>
            <div className="flex  items-center space-x-4 ">
                <Listbox value={tokenPair} onChange={OnChangePair}>
                    ...
                    >
                        <img src={tokenPair.logo1} alt={tokenPair.name1} className="w-6 h-6" />
                        <span className="text-sm/6 text-white ml-1">{tokenPair.name1}</span>
                        <span className="text-sm/6 text-white mx-2">/</span>
                        <img src={tokenPair.logo2} alt={tokenPair.name2} className="w-6 h-6" />
                        <span className="text-sm/6 text-white ml-1">{tokenPair.name2}</span>
                    ...
                    >
                        {pairs.filter((token) => token.id !== tokenPair.id).map((token) => (
                            <ListboxOption
                                key={token.id}
                                value={token}
                                className="group flex items-center gap-2 cursor-default rounded-lg py-1.5 px-3 select-none data-[focus]:bg-white/10"
                            >
                                <img src={token.logo1} alt={token.name1} className="w-6 h-6" />
                                <span className="text-sm/6 text-white ml-1">{token.name1}</span>
                                <span className="text-sm/6 text-white mx-2">/</span>
                                <img src={token.logo2} alt={token.name2} className="w-6 h-6" />
                                <span className="text-sm/6 text-white ml-1">{token.name2}</span>
                                <CheckIcon className="invisible size-4 fill-white group-data-[selected]:visible" />
                            </ListboxOption>
                        ))}
                    </ListboxOptions>
